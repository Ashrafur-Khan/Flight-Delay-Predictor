...
- The Windows release artifact is an NSIS installer `.exe` generated by the same desktop packaging flow.
...
- repo-root `package.json`
  Release orchestration entrypoint for Electron packaging commands.
...
- Windows packaging writes an NSIS `.exe` to `desktop/dist/installers/`
...
1. Run `npm run build:desktop` on Windows.
2. Upload the generated `.exe` from `desktop/dist/installers/` to the GitHub release.
3. Optionally run `node desktop/scripts/prepare-release-assets.mjs --extension=.exe` to generate a matching `.sha256` file.
...
- The desktop app is verified locally but is not yet polished for public distribution. Releases are still manual, and it still needs custom icons, production signing, and macOS notarization.
